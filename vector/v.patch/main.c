    ...
    *colnames = '\0';
    ...
    if (do_table) {
        ...
        while (old->answers[i]) {
            ...
            if (fi_in) {
                ...
                G_fatal_error(_("Missing attribute table for vector map <%s>"),
                              in_name);
            }
            ...
            if (i > 0 || append->answer) {
                ...
                if (G_strcasecmp(fi_in->key, key) != 0) {
                    G_fatal_error(_("Key (category) column names differ:"
                                    " <%s> from <%s> and <%s> from <%s>"),
                                  fi_in->key, in_name, key, meta_name);
                }
    ...
    if (do_table) {
        ...
        if (!append->answer) {
            ...
            if (db_create_index2(driver_out, fi_out->table, fi_out->key) !=
                DB_OK)
