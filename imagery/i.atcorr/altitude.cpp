...
void Altitude::pressure(AtmosModel &atms, double &uw, double &uo3)
{
    ...
    double xa = (double)((atms.z[isup] - atms.z[iinf]) /
                         log(atms.p[isup] / atms.p[iinf]));
    ...
    double xalt = xps;
    double xtemp =
        (atms.t[isup] - atms.t[iinf]) / (atms.z[isup] - atms.z[iinf]);
    xtemp = xtemp * (xalt - atms.z[iinf]) + atms.t[iinf];
    double xwo =
        (atms.wo[isup] - atms.wo[iinf]) / (atms.z[isup] - atms.z[iinf]);
    xwo = xwo * (xalt - atms.z[iinf]) + atms.wo[iinf];
    double xwh =
        (atms.wh[isup] - atms.wh[iinf]) / (atms.z[isup] - atms.z[iinf]);
    ...
    for (i = l; i < 34; ++i) {
        ...
        atms.wh[i] =
            (atms.wh[33] - atms.wh[l]) * (i - l) / (33 - l) + atms.wh[l];
        atms.wo[i] =
            (atms.wo[33] - atms.wo[l]) * (i - l) / (33 - l) + atms.wo[l];
    }
...
/*
  Function: Update the atmospheric profile (P(z),T(z),H2O(z),O3(z)) in case the
  observer is on board an aircraft.

  Description: Given the altitude or pressure at aircraft level as input, the
  first task is to compute the altitude (in case the pressure has been entered)
  or the pressure (in case the altitude has been entered) at plane level. Then,
  a new atmospheric profile is created (Pp,Tp,H2Op,O3p) for which the last level
  is located at the plane altitude. This profile is used in the gaseous
  absorption computation (ABSTRA.f) for the path from target to sensor (upward
  transmission). The ozone and water vapor integrated content of the "plane"
  atmospheric profile are also an output of this subroutine. The last output is
  the proportion of molecules below plane level which is useful in scattering
  computations (OS.f,ISO.f).
*/
void Altitude::presplane(AtmosModel &atms)
{
    ...
    int k;
    for (k = 0; k < 33; k++) {
        double roair = (double)(air * 273.16 * plane_sim.ppl[k] /
    ...
    for (k = 1; k < 33; k++) {
        double ds =
            (plane_sim.ppl[k - 1] - plane_sim.ppl[k]) / plane_sim.ppl[0];
...
void Altitude::print()
{
    if (palt < 1000) {
        ...
        s2.precision(3);
        s2 << " plane  altitude absolute [km] " << setw(9) << plane_sim.zpl[33]
           << ends;
