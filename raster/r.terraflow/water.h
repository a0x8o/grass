...
class labelElevTypePrintLabel {
public:
    cclabel_type operator()(const labelElevType &p) { return p.label; }
};
...
>>>>>>> 6cf60c76a4 (wxpyimgview: explicit conversion to int (#2704))
    ...
    {
        return s << "[waterType" << (ijBaseType)p << ", dir=" << p.dir
                 << ", bfs=" << p.depth << ", lab=" << p.label << "]";
    }
};
...
public:
    compressedWaterWindowBaseType() : depth(DEPTH_INITIAL), dir(0)
    {
        for (int i = 0; i < 9; i++) {
            el[i] = nodataType::ELEVATION_NODATA;
        }
    };
    compressedWaterWindowBaseType(dimension_type gi, dimension_type gj,
    ...
    elevation_type getElevation(int i, int j) const
    {
        return el[linear(i, j)];
    };
    direction_type getDirection() const { return dir; };
    ...
    bfs_depth_type getDepth(int i, int j) const
    {
        return getDepth(norm(i, j));
    };
    void sanityCheck();
...
private:
    int computeDelta(waterWindowBaseType *center, int index,
...
>>>>>>> 6cf60c76a4 (wxpyimgview: explicit conversion to int (#2704))
    compressedWaterWindowType(dimension_type gi, dimension_type gj,
        ...
        : compressedWaterWindowBaseType(gi, gj, a, b, c), label(glabel)
    {
...
void createWaterWindows(AMI_STREAM<waterGridType> *mergedWaterStr,
...
void findBoundaries(AMI_STREAM<labelElevType> *labeledWater,
                    const dimension_type nrows, const dimension_type ncols,
                    AMI_STREAM<boundaryType> *boundaryStr);
