...
template <class T>
void printStream(ostream &s, AMI_STREAM<T> *str)
{
    T *elt;
    AMI_err ae;
...
void printStream2Grid(AMI_STREAM<T> *str, dimension_type nrows,
                      dimension_type ncols, const char *name, FUN fmt)
{
    T *elt, nodata;
    AMI_err ae;
    ofstream fstrm(name);
...
template <class T, class FUN>
void printGridStream(AMI_STREAM<T> *str, dimension_type nrows,
                     dimension_type ncols, char *name, FUN fmt)
{
    T *elt;
    AMI_err ae;
    ofstream fstrm(name);
...
/* assume sorted... */
template <class T, class FUN>
AMI_STREAM<T> *removeDuplicates(AMI_STREAM<T> *str, FUN fo)
{
    AMI_err ae;

    AMI_STREAM<T> *newStr = new AMI_STREAM<T>();
    if (str->stream_len() == 0)
        return newStr; /* empty stream */
...
template <class T, class FUN>
void removeDuplicatesEx(AMI_STREAM<T> **str, FUN fo)
{
...
AMI_STREAM<T> *mergeStream2Grid(AMI_STREAM<T> *grid, dimension_type rows,
                                dimension_type cols, AMI_STREAM<TT> *str,
                                FUN fo)
{
    AMI_err ae, aeS;
    T *gep;  /* grid element */
    ...
    for (dimension_type i = 0; i < rows; i++) {
        for (dimension_type j = 0; j < cols; j++) {
            ...
            if ((aeS == AMI_ERROR_NO_ERROR) && (sep->i == i) && (sep->j == j)) {
                ...
                assert(ae == AMI_ERROR_NO_ERROR);
                aeS = str->read_item(&sep);
                assert(aeS == AMI_ERROR_NO_ERROR ||
                       aeS == AMI_ERROR_END_OF_STREAM);
            }
            else {
