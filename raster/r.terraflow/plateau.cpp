...
public:
    ...
    void processWindow(dimension_type row, dimension_type col,
                       elevation_type *a, elevation_type *b, elevation_type *c);
...
detectPlateaus::detectPlateaus(const dimension_type gnrows,
    ...
    : dirStream(gdirstr), nrows(gnrows), ncols(gncols),
      nodata_value(gnodata_value)
{
...
>>>>>>> a55c458ec3 (wxpyimgview: explicit conversion to int (#2704))
{
    ...
    platQueue->peek(0, &pt);
    cout << "queue length = " << platQueue->length() << "; head=" << pt << endl;
#endif
    return ptarr;
}
...
direction_type *detectPlateaus::getDirectionForward(dimension_type i,
                                                    dimension_type j,
                                                    dimension_type nr,
                                                    dimension_type nc)
{
    ...
    if (i >= 0) {
        if (!(i == 0 && j == -1))
            dirQueue->dequeue(dirarr);
        if (j == -1)
            dirarr[0] = 0;
        if (j + 1 < nc)
            dirQueue->peek(0, dirarr + 1);
        if (j + 2 < nc)
            dirQueue->peek(1, dirarr + 2);
    }

#if (0)
    cout << "\t\t\trequest at " << i << "," << j << " returns: " << dirarr[0]
         << " " << dirarr[1] << " " << dirarr[2] << "\t";
    direction_type dir;
    dirQueue->peek(0, &dir);
    cout << "queue length = " << dirQueue->length() << "; head=" << dir << endl;
#endif
...
/* ********************************************************************** */
void detectPlateaus::processWindow(dimension_type row, dimension_type col,
                                   ...
                                   elevation_type *c)
{
    ...
    if (col == 0)
        prevPlat.cclabel = LABEL_UNDEF; /* no left cell */

    /* now check for continuing plateaus */
    plateauType *ptarr = getPlateauForward(row - 1, col - 1, nrows, ncols);
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (i < 3) {
            if (ptarr[i].valid)
                label = ptarr[i].cclabel;
        }
        else {
            if (prevPlat.valid)
                label = prevPlat.cclabel;
        }
...
>>>>>>> a55c458ec3 (wxpyimgview: explicit conversion to int (#2704))
    for (int i = 0; i < 4; i++) {
        ...
        if (ndir > 0) { /* has direction */
            ...
            if ((nbor.i >= 0) &
                (nbor.j >=
                 0)) { /* make sure nbor on grid;
                                                                  this can
                          happen because we pad the grid with nodata */
...
/* ********************************************************************** */
void detectPlateaus::generatePlateaus(AMI_STREAM<elevation_type> &elstr)
{
...
void detectPlateaus::removeDuplicates()
{
...
void detectPlateaus::relabelPlateaus()
{
    AMI_err ae;
    plateauType *pt;
...
void detectPlateaus::generateStats(AMI_STREAM<plateauStats> *statStr)
{
    AMI_err ae;
    plateauType *pt;
...
#if (0)
    { /* XXX */
        AMI_STREAM<plateauType> *tmp =
            sort(md.getPlateaus(), ijCmpPlateauType());
        printStream2Grid(tmp, nrows, ncols, "label0.asc",
                         plateauType::printLabel);
        delete tmp;
    }
#endif
