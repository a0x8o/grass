...
/* if viewOptions.doCurv is on then adjust the passed height for
   ...
   unchanged.
 */
float adjust_for_curvature(Viewpoint vp, double row, double col, float h,
...
size_t init_event_list_in_memory(AEvent *eventList, char *rastName,
...
void save_grid_to_GRASS(Grid *grid, char *filename, RASTER_MAP_TYPE type,
                        OutputMode mode);
...
void save_vis_elev_to_GRASS(Grid *visgrid, char *elevfname, char *visfname,
                            float vp_elev);
...
void save_io_visibilitygrid_to_GRASS(IOVisibilityGrid *visgrid, char *outfname,
                                     RASTER_MAP_TYPE type, float (*fun)(float),
                                     OutputMode mode);
...
/*  using the visibility information recorded in visgrid, it creates
   ...
   (i,j) order. */
void save_io_vis_and_elev_to_GRASS(IOVisibilityGrid *visgrid, char *elevfname,
