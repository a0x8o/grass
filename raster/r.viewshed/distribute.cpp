                                       ...
                                       const ViewOptions &viewOptions)
{
    ...
    G_verbose_message(
        "Total cells %ld, visible cells %ld (%.1f percent).",
                                ...
                                GridHeader *hd, const ViewOptions &viewOptions)
{
    ...
    G_debug(2, "***  DISTRIBUTE sector [%.4f, %.4f]  ***", start_angle,
            end_angle);
    ...
    *******************************************************/
    if (eventList->stream_len() * sizeof(AEvent) <
        ...
        if (enterBndEvents) {
            nvis += solve_in_memory(eventList, enterBndEvents, start_angle,
                                    end_angle, visgrid, hd, vp, viewOptions);
            return nvis;
        }
        else {
    ...
    *******************************************************/
    ...
    for (off_t i = 0; i < nbEvents; i++) {
        ...
        if (is_almost_on_boundary(e->angle, s, start_angle, end_angle, nsect)) {
            ...
            G_debug(2, "angle=%f close to sector boundaries=[%f, %f]", e->angle,
                    s * ssize, (s + 1) * ssize);
        }

        G_debug(2, "event %7lu: ", (unsigned long)i);
        print_event(*e, 2);
        G_debug(2, "d=%8.1f, ", get_square_distance_from_viewpoint(*e, *vp));
        G_debug(2, "s=%3d ", s);
        ...
        case ENTERING_EVENT:
            ...
            if (exit_s == s) {
                ...
                /*semi-short event; insert in sector s, and in sector boundary
                   s+1 NOTE: to avoid precision issues, the events are inserted
                ...
                longEvents++;
                /*insertEventInSector(e, exit_s, &sectorBnd[exit_s],
                   high[exit_s],
        ...
        case EXITING_EVENT:
            ...
            if (enter_s == s) {
                ...
                /*semi-short event
                   //the corresponding ENTER event must insert itself in
                   sectorBnd[s] */
                ...
                insert_event_in_sector(e, s, &sectorBnd[s], high[s], vp,
                                       bndInsert, bndDrop);
            }
            else {
                /*long event */
    ...
    if (enterBndEvents)
        distribute_bnd_events(enterBndEvents, sectorBnd, nsect, vp, start_angle,
                              end_angle, high, bndInsert, bndDrop);

    /*sanity checks */
    G_debug(2, "boundary events in distribution: %ld", boundaryEvents);
    print_sector_stats(nbEvents, nsect, high, total, insert, drop, sector,
                       sectorBnd, bndInsert, longEvents, start_angle,
                       end_angle);
    ...
    for (int i = 0; i < nsect; i++) {
        ...
        G_debug(2, "saving BndStr %d: %s", i, sectorBndName[i]);
    ...
    for (int i = 0; i < nsect; i++) {
        ...
        AMI_STREAM<AEvent> *str = new AMI_STREAM<AEvent>(sectorName[i]);
        assert(str);
        G_debug(3, " len=%lu", (unsigned long)str->stream_len());
        /*recover boundary stream */
        G_debug(3, "opening boundary sector stream %s ", sectorBndName[i]);
        AMI_STREAM<AEvent> *bndStr = new AMI_STREAM<AEvent>(sectorBndName[i]);
        assert(str);
        G_debug(3, " len=%lu", (unsigned long)bndStr->stream_len());

        nvis += distribute_sector(
            str, bndStr, start_angle + i * ((end_angle - start_angle) / nsect),
    ...
    G_debug(2, "Distribute sector [ %.4f, %.4f] done.", start_angle, end_angle);
                           ...
                           double *high, long *insert, long *drop)
{
    ...
    G_debug(3, "Distribute boundary of sector [ %.4f, %.4f] ", start_angle,
            end_angle);
    ...
    for (off_t i = 0; i < nbEvents; i++) {
        ...
        insert_event_in_sector(e, exit_s, &sectorBnd[exit_s], high[exit_s], vp,
                               insert, drop);

    } /*for i */

    G_debug(3, "Distribute boundary of sector [ %.4f, %.4f] done.", start_angle,
            end_angle);
    ...
    for (off_t i = 0; i < nbEvents; i++) {
        ...
        case EXITING_EVENT:
            ...
            SOLVEINMEMDEBUG
            {
                ...
                /*find its corresponding enter event and its sector */
                double enter_angle = calculate_enter_angle(e->row, e->col, vp);
        ...
        case CENTER_EVENT:
            ...
            if (max <= sn.gradient[1]) {
                ...
                /* else the point is invisible; we do not write it to the
                   //visibility stream, because we only record visible and
                   nodata
...
{
    return (angle >= (start_angle - EPSILON) && angle <= (end_angle + EPSILON));
}
                       ...
                       AEvent *e, double *high)
{
    ...
    if (start_s == -1) {
        ...
        for (int j = 0; j < end_s; j++) {
            if (high[j] < ctrgrad) {
                /*printf("update high[%d] from %.2f to %.2f ", j, high[j],
                 * ctrgrad); */
                high[j] = ctrgrad;
            }
        }
                        ...
                        long longEvents, double start_angle, double end_angle)
{
    ...
    G_debug(3, "Distribute [%.4f, %.4f]: nsect=%d, ", start_angle, end_angle,
            nsect);
    G_debug(3, "total events %lu, inserted %lu, dropped %lu, long events=%ld",
                     ...
                     int nsect)
{
    ...
    /*printf("getsector: fit %.2f in (%.2f. %.2f)", angle, sstartAngle,
       sendAngle);
