...
int dir_degree(int dir_fd, int val_fd, struct point *startp,
               struct Cell_head *window, struct Map_info *Out,
               struct point_list *pl, int out_mode);
int dir_bitmask(int dir_fd, int val_fd, struct point *startp,
                struct Cell_head *window, struct Map_info *Out,
                struct point_list *pl, int out_mode);
...
int main(int argc, char **argv)
{
    ...
    opt.val->key = "values";
    opt.val->label = _("Name of input raster values to be used for output");
    opt.val->required = NO;
...
>>>>>>> 8422103f4c (wxpyimgview: explicit conversion to int (#2704))
    ...
    if (opt.coord->answer) {
        for (i = 0; opt.coord->answers[i] != NULL; i += 2) {
            ...
            next_start_pt = (struct point *)(G_malloc(sizeof(struct point)));
...
>>>>>>> 61b5b5fabe (r.horizon manual - fix typo (#2794))
    ...
    if (strcmp(opt.format->answer, "degree") == 0) {
        ...
        if (dmax > (1 << 16) - 1)
            G_fatal_error(_("Bitmask encoded directions can not be > %d"),
                          (1 << 16) - 1);
        dir_format = DIR_BIT;
    }
