...
int main(int argc, char *argv[])
{
    ...
    grid_opt->key = "grid";
    grid_opt->description =
        _("Name for output vector map with interpolation grid");
...
>>>>>>> osgeo-main
    ...
    if (G_projection() == PROJECTION_LL) {
        ...
        while (src_reg.west >= dest_reg.east &&
               src_reg.east - 360.0 > dest_reg.west) {
            src_reg.east -= 360.0;
            src_reg.west -= 360.0;
        }
        while (src_reg.east <= dest_reg.west &&
               src_reg.west + 360.0 < dest_reg.east) {
            src_reg.east += 360.0;
            src_reg.west += 360.0;
        }
    }
    ...
    in_file = G_tempfile();
    if (Segment_open(&in_seg, in_file, nrows, ncols, SEGSIZE, SEGSIZE,
    ...
    if (have_mask) {
        ...
        if (Segment_open(&mask_seg, mask_file, nrows, ncols, SEGSIZE, SEGSIZE,
                         sizeof(char), segments_in_memory) != 1)
            G_fatal_error(_("Can not create temporary file"));
    ...
    while (last_row == FALSE) { /* For each subregion row */
        ...
        while (last_column == FALSE) { /* For each subregion column */
            ...
            if (npoints > 0 && npoints_marked > 0) { /*  */
                ...
                BW = P_get_BandWidth(interp_method,
