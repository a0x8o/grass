...
class TestTemporalRasterAlgebraSpatialTopology(TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.runModule(
            "g.region", n=50.0, s=20.0, e=50.0, w=20.0, t=1.0, b=0.0, res=10.0
        )
        ...
        cls.runModule(
            "g.region", n=40.0, s=25.0, e=40.0, w=25.0, t=1.0, b=0.0, res=10.0
        )
        ...
        cls.runModule(
            "g.region", n=60.0, s=40.0, e=60.0, w=40.0, t=1.0, b=0.0, res=10.0
        )
        ...
        cls.runModule(
            "r.mapcalc", overwrite=True, quiet=True, expression="singletmap = 100"
        )
        cls.runModule(
            "r.mapcalc", overwrite=True, quiet=True, expression="singlemap = 1000"
        )
    ...
    def test_equal_equivalent_sum(self) -> None:
        ...
        ta.parse(
            expression="R = A {+,equal|equivalent,l} B", basename="r", overwrite=True
        )
    ...
    def test_equal_overlap_sum_with_null(self) -> None:
        ...
        """
        ta = tgis.TemporalRasterAlgebraParser(
            run=True, debug=True, spatial=True, register_null=True
        )
        ta.parse(expression="R = A {+,equal|overlap,l} B", basename="r", overwrite=True)
    ...
    def test_equal_equivalent_contain_sum(self) -> None:
        ...
        ta = tgis.TemporalRasterAlgebraParser(run=True, debug=True, spatial=True)
        ta.parse(
            expression="R = A {+,equal|equivalent|contain,l} B",
            basename="r",
            overwrite=True,
        )
    ...
    def test_equal_equivalent_compare(self) -> None:
        ...
        ta = tgis.TemporalRasterAlgebraParser(run=True, debug=True, spatial=True)
        ta.parse(
            expression="R = if({equal|equivalent}, A > 0 {&&,equal|equivalent} B < 10, A {+,equal|equivalent,l} B)",
            basename="r",
            overwrite=True,
        )
